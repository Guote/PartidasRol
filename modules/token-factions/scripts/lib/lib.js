...
function advancedLosTestVisibility(sourceToken, token, source) {
    ...
    if (sourceToken.vision.los === source)
        return inLOS;
...
function testInAngle(sourceToken, token) {
    ...
    //check angled vision
    const angle = normalizeAngle(Math.atan2(token.center.y - sourceToken.center.y, token.center.x - sourceToken.center.x));
...
function tokenInRange(sourceToken, token) {
    ...
    const dist = (getUnitTokenDist(sourceToken, token) * canvas.dimensions?.size) / canvas.dimensions?.distance -
        tokensSizeAdjust;
    return dist <= range;
}
...
function testCollision(p0, p1, type = "sight") {
    ...
    for (let tile of canvas.tiles?.placeables) {
        if (tile.document.flags?.levels?.noCollision)
            continue;
        ...
        if (bottom != -Infinity) {
            ...
            if (((z0 < bottom && bottom < z1) || (z1 < bottom && bottom < z0)) &&
    ...
    function walls3dTest() {
        ...
        for (let wall of walls) {
            ...
            if (wall.direction !== null) {
                ...
                //@ts-ignore
                if (!wall.isDirectionBetweenAngles(...angleBounds))
                    continue;
            }
