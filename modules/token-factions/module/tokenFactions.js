...
export class TokenFactions {
    ...
    static AddBorderToggle(app, html, data) {
        ...
        const factionDisableFlag = app.object.document.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_DISABLE);
        const borderButton = `
    ...
    static async ToggleBorder(event) {
        //@ts-ignore
        const borderIsDisabled = this.object.document.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_DISABLE);
        for (const token of canvas.tokens?.controlled) {
            //@ts-ignore
            await token.document.setFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_DISABLE, !borderIsDisabled);
            if (borderIsDisabled) {
                await token.document.unsetFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_COLOR_INT);
                await token.document.unsetFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_COLOR_EXT);
                await token.document.unsetFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_FRAME_OPACITY);
                await token.document.unsetFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_BASE_OPACITY);
            }
        }
    ...
    static async ToggleCustomBorder(event) {
        ...
        const currentCustomColorTokenInt = tokenTmp.document.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_COLOR_INT) ||
            '#000000';
        const currentCustomColorTokenExt = tokenTmp.document.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_COLOR_EXT) ||
            '#000000';
        const currentCustomColorTokenFrameOpacity = tokenTmp.document.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_FRAME_OPACITY) || 0.5;
        const currentCustomColorTokenBaseOpacity = tokenTmp.document.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_BASE_OPACITY) || 0.5;
        const dialogContent = `
        ...
        const d = new Dialog({
            ...
            buttons: {
                yes: {
                    ...
                    callback: async (html) => {
                        const newCurrentCustomColorTokenInt = ($(html.find(`input[data-edit='token-factions.currentCustomColorTokenInt']`)[0]).val());
                        const newCurrentCustomColorTokenExt = ($(html.find(`input[data-edit='token-factions.currentCustomColorTokenExt']`)[0]).val());
                        const newCurrentCustomColorTokenFrameOpacity = ($(html.find(`input[data-edit='token-factions.currentCustomColorTokenFrameOpacity']`)[0]).val());
                        const newCurrentCustomColorTokenBaseOpacity = ($(html.find(`input[data-edit='token-factions.currentCustomColorTokenBaseOpacity']`)[0]).val());
                        for (const token of canvas.tokens?.controlled) {
                            token.document.setFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_COLOR_INT, newCurrentCustomColorTokenInt);
                            token.document.setFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_COLOR_EXT, newCurrentCustomColorTokenExt);
                            token.document.setFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_FRAME_OPACITY, newCurrentCustomColorTokenFrameOpacity);
                            token.document.setFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_BASE_OPACITY, newCurrentCustomColorTokenBaseOpacity);
                        }
                    },
                },
                no: {
    ...
    static drawBorder(token, borderColor, factionBorder, fillTexture) {
        ...
        const customFrameOpacity = (token.document.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_FRAME_OPACITY));
        const customBaseOpacity = (token.document.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_BASE_OPACITY));
        ...
        //@ts-ignore
            ...
            //@ts-ignore
            const polygon = canvas.grid?.grid?.getPolygon(-1.5 - q + sW, -1.5 - q + sH, (token.w + 2) * s + p, (token.h + 2) * s + p);
            if (fillTexture) {
                //@ts-ignore
                factionBorder
                    .beginFill(borderColor.EX, baseOpacity)
...
TokenFactions.renderTokenConfig = async function (config, html) {
    ...
    //       false;
    const factionDisableValue = config.object.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_DISABLE)
        ? 'checked'
        : '';
    const currentCustomColorTokenInt = config.object.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_COLOR_INT) ||
        '#000000';
    const currentCustomColorTokenExt = config.object.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_COLOR_EXT) ||
        '#000000';
    const currentCustomColorTokenFrameOpacity = config.object.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_FRAME_OPACITY) ||
        0.5;
    const currentCustomColorTokenBaseOpacity = config.object.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_BASE_OPACITY) ||
        0.5;
    // Expand the width
...
TokenFactions._applyFactions = async function (document, updateData) {
