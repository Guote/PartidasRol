...
export let setting = (key) => {
  ...
  else return game.settings.get("monks-bloodsplats", key);
};
...
export class MonksBloodsplats {
  ...
  static init() {
    ...
    MonksBloodsplats.availableGlyphs =
      "!\"#$%&'()*+,-./01234568:;<=>?@ABDEFGHIKMNOPQRSTUVWX[\\]^_`acdfhoquvx|}~¢£¥§©ª«¬®°±¶·º¿ÀÁÂÄÅÆÈÉÊËÌÏÑÒÓÔÖØÙÚÜßàáâåæçéêëìíîïñòõ÷øùûüÿiœŸƒπ";

    MonksBloodsplats.splatfont = new FontFace(
      "WC Rhesus A Bta",
      "url('modules/monks-bloodsplats/fonts/WCRhesusABta.woff2')"
    );
  ...
  static async transferSettings() {
    let setSetting = async function (name) {
      let oldChange = game.settings.settings.get(
        `monks-bloodsplats.${name}`
      ).onChange;
      game.settings.settings.get(`monks-bloodsplats.${name}`).onChange = null;
      await game.settings.set(
        "monks-bloodsplats",
        name,
        game.settings.get("monks-little-details", name)
      );
      game.settings.settings.get(`monks-bloodsplats.${name}`).onChange =
        oldChange;
    };
    ...
    for (let scene of game.scenes) {
      for (let token of scene.tokens) {
        if (
          getProperty(token, "flags.monks-little-details.bloodsplat-colour")
        ) {
          await token.update({
            "flags.monks-bloodsplats.bloodsplat-colour": getProperty(
              token,
              "flags.monks-little-details.bloodsplat-colour"
            ),
          });
        }
      }
    ...
    ui.notifications.warn(
      "Monk's Bloodsplats has transfered over settings from Monk's Little Details, you will need to refresh your browser for some settings to take effect.",
      { permanent: true }
    );
  ...
  static isDefeated(token) {
    return (
      ...
      token.actor?.effects.find(
        (e) =>
          e.getFlag("core", "statusId") === CONFIG.specialStatusEffects.DEFEATED
      ) ||
...
Hooks.on("renderSettingsConfig", (app, html, data) => {
  ...
  $("<input>")
    .attr("type", "color")
    .attr("data-edit", "monks-bloodsplats.bloodsplat-colour")
    .val(colour)
    .insertAfter(
      $('input[name="monks-bloodsplats.bloodsplat-colour"]', html).addClass(
        "color"
      )
    );
...
Hooks.on("createCombatant", function (combatant, data, options) {
  ...
  if (setting("show-bloodsplat") != "false" && combatant && game.user.isGM) {
    ...
    if (token) {
      ...
      if (glyph == undefined) {
        glyph = MonksBloodsplats.availableGlyphs.charAt(
          Math.floor(Math.random() * MonksBloodsplats.availableGlyphs.length)
        );
...
Hooks.on("updateCombat", async function (combat, delta) {
  let combatStarted =
    combat && delta.round === 1 && combat.turn === 0 && combat.started === true;

  //set the bloodsplat glyph when the combat starts to maintain consistency
  if (
    setting("show-bloodsplat") != "false" &&
    game.user.isGM &&
    combatStarted
  ) {
...
Hooks.on("renderTokenConfig", (app, html, data) => {
  if (game.user.isGM) {
    let colour = getProperty(
      app.token,
      "flags.monks-bloodsplats.bloodsplat-colour"
    );
        ...
        $("<div>")
          ...
          .append(
            $("<input>")
              .addClass("color")
              .attr("type", "text")
              .attr("name", "flags.monks-bloodsplats.bloodsplat-colour")
              .val(colour)
          )
          .append(
            $("<input>")
              .attr("type", "color")
              .attr("data-edit", "flags.monks-bloodsplats.bloodsplat-colour")
              .val(colour)
          )
      )
...
Hooks.on("refreshToken", (token) => {
  //find defeated state
  if (MonksBloodsplats.isDefeated(token)) {
    token.bars.visible = false;
    for (let effect of token.effects.children) {
      effect.alpha = 0;
    }
    if (
      ["dnd5e.LootSheetNPC5e", "core.MerchantSheet"].includes(
        token.actor?.flags?.core?.sheetClass
      ) ||
      token.actor?.flags["item-piles"]?.data?.enabled == true
    ) {
      ...
      if (token.document._id != undefined) {
        if (token.bloodsplat?.transform == undefined) {
          ...
          if (token.bloodsplat) token.bloodsplat.destroy();

          let glyph = token.document.getFlag("monks-bloodsplats", "glyph");
          if (glyph == undefined) {
            glyph = MonksBloodsplats.availableGlyphs.charAt(
              Math.floor(
                Math.random() * MonksBloodsplats.availableGlyphs.length
              )
            );
            if (game.user.isGM)
          ...
          let colour =
            token.document.getFlag("monks-bloodsplats", "bloodsplat-colour") ||
            setting("bloodsplat-colour") ||
            "0xff0000";
          token.bloodsplat = new PIXI.Text(" " + glyph + " ", {
            fontFamily: "WC Rhesus A Bta",
            fontSize: token.h * setting("bloodsplat-size"),
            fill: colour,
            align: "center",
          });
          token.bloodsplat.alpha =
            animate || (token.document.hidden && !game.user.isGM) ? 0 : 0.7;
          ...
          const iconAlpha =
            game.user.isGM ||
            (setting("show-bloodsplat") == "both" && !token.document.hidden)
              ? setting("bloodsplat-opacity")
              : 0;
          if (animate) {
          ...
          } else token.mesh.alpha = iconAlpha;
        } else {
          const iconAlpha =
            game.user.isGM ||
            (setting("show-bloodsplat") == "both" && !token.document.hidden)
              ? setting("bloodsplat-opacity")
              : 0;
          if (token._animateTo != iconAlpha) token.mesh.alpha = iconAlpha;
          token.bloodsplat.position.set(
            token.x + token.w / 2,
            token.y + token.h / 2
          );
          token.bloodsplat.visible = token.isVisible;
        }
        if (token.tresurechest != undefined) token.tresurechest.alpha = 0;
      }
    }
  } else {
    if (token.bloodsplat) {
...
Hooks.on("sightRefresh", function () {
  for (let token of canvas.tokens.placeables) {
    if (token.bloodsplat) token.bloodsplat.visible = token.isVisible;
  }
});

Hooks.on("drawGridLayer", function (layer) {
  layer.bloodsplats = layer.addChildAt(
    new PIXI.Container(),
    layer.ldmarkers?.parent
      ? layer.getChildIndex(layer.ldmarkers)
