...
export class CombatAttackDialog extends FormApplication {
    ...
    activateListeners(html) {
        ...
        html.find('.send-attack').click(() => {
            const { weapon, criticSelected, modifier, fatigueUsed, damage, weaponUsed, unarmed } = this.modalData.attacker.combat;
        ...
        html.find('.send-mystic-attack').click(() => {
            const { magicProjectionType, spellGrade, spellUsed, modifier, critic, damage } = this.modalData.attacker.mystic;
            if (spellUsed) {
    ...
    getData() {
        const { attacker: { combat, psychic }, ui } = this.modalData;
        ui.hasFatiguePoints =
