...
export class GMCombatDialog extends FormApplication {
  ...
  get isDamagingCombat() {
    ...
    const isPsychicDamagingCombat =
      ...
      attacker.result.values.critic !== NoneWeaponCritic.NONE;
    return (
      isPhysicalDamagingCombat ||
      isMysticDamagingCombat ||
      isPsychicDamagingCombat
    );
  }
  get canApplyDamage() {
    ...
    const hasDamage =
      calculations.damage !== undefined && calculations?.damage > 0;
  ...
  activateListeners(html) {
    ...
    html.find(".make-counter").click(() => {
      ...
      if (this.modalData.calculations?.canCounter) {
        this.hooks.onCounterAttack(
          this.modalData.calculations.counterAttackBonus
        );
      }
    });
    ...
    html.find(".show-results").click(async () => {
      ...
      await renderTemplate(Templates.Chat.CombatResult, data).then(
        (content) => {
          ChatMessage.create({
            content,
          });
  ...
  applyValuesIfBeAble() {
    ...
    if (this.modalData.attacker.result?.type === "combat") {
      this.attackerActor.applyFatigue(
        this.modalData.attacker.result.values.fatigueUsed
      );
    }
    if (this.modalData.defender.result?.type === "combat") {
      this.defenderActor.applyFatigue(
        this.modalData.defender.result.values.fatigue
      );
    }
  }
}
