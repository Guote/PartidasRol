import { debug, i18n } from "./lib/lib.js";
...
export class TokenFactions {
    ...
    static async ToggleCustomBorder(event) {
        ...
        const currentCustomColorTokenInt = tokenTmp.document.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_COLOR_INT) || "#000000";
        const currentCustomColorTokenExt = tokenTmp.document.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_COLOR_EXT) || "#000000";
    ...
    static drawBorderFaction(token, container) {
        //@ts-ignore
        ...
        try {
            ...
            //);
            skipDraw = getProperty(token.document, `flags.${CONSTANTS.MODULE_NAME}.${TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_DISABLE}`);
        }
        catch (e) {
            //@ts-ignore
    ...
    static drawBorder(token, borderColor, container, fillTexture) {
        // //@ts-ignore
        ...
        //@ts-ignore
            ...
            //@ts-ignore
            const polygon = canvas.grid?.grid?.getPolygon(-1.5 - q + sW, -1.5 - q + sH, (token.w + 2) * sX + p, (token.h + 2) * sY + p);
            //@ts-ignore
    ...
    static drawGradient(token, color, bevelGradient) {
    ...
    static drawTexture(token, color, bevelTexture) {
...
TokenFactions.renderTokenConfig = async function (config, html) {
    ...
    const currentCustomColorTokenExt = config.object.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_COLOR_EXT) ||
        "#000000";
    const currentCustomColorTokenFrameOpacity = config.object.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_FRAME_OPACITY) || 0.5;
    const currentCustomColorTokenBaseOpacity = config.object.getFlag(CONSTANTS.MODULE_NAME, TokenFactions.TOKEN_FACTIONS_FLAGS.FACTION_CUSTOM_BASE_OPACITY) || 0.5;
    // Expand the width
