...
export class CombatTurn {
  ...
  static init() {
    ...
    if (setting("remember-previous")) {
      let combatNextTurn = async function (wrapped, ...args) {
        ...
        if (current?.isOwner) {
          ...
          if (game.user.isGM)
            await current.document.setFlag(
      ...
      if (game.modules.get("lib-wrapper")?.active) {
        ...
        Combat.prototype.nextTurn = function (event) {
          return combatNextTurn.call(
            this,
            oldNextTurn.bind(this),
            ...arguments
          );
        };
      }
    ...
    Hooks.on("createCombatant", function (combatant, data, options) {
      ...
      if (combatant.actor?.isOwner == true) CombatTurn.checkCombatTurn(combat);
    });

    Hooks.on("deleteCombat", function (combat) {
      if (
        setting("round-chatmessages") &&
        combat &&
        game.user.isTheGM &&
        combat.started
      ) {
        ChatMessage.create(
          { user: game.user, flavor: "Round End" },
          { roundmarker: true }
        );
      }
    ...
    Hooks.on("updateCombat", async function (combat, delta) {
      if (delta.turn != undefined) CombatTurn.checkCombatTurn(combat);
      ...
      if (
        ...
        combat.started &&
        game.user.isGM &&
        setting("pan-to-combatant") &&
        combat?.combatant?.token
      ) {
        if (
          canvas.dimensions.rect.contains(
            combat?.combatant?.token.x,
            combat?.combatant?.token.y
          )
        ) {
          canvas.animatePan({
            x: combat?.combatant?.token.x,
            y: combat?.combatant?.token.y,
          });
        }
      }
      ...
      if (
        combat &&
        combat.started &&
        setting("remember-previous") &&
        combat?.combatant?.token?.isOwner
      ) {
        let targets = [];
        if (game.user.isGM)
          targets = combat.combatant.token.getFlag(
            "monks-combat-details",
            "targets"
          );
        else targets = game.user.getFlag("monks-combat-details", "targets");
        ...
        if (targets && targets.length > 0) {
          for (let id of targets) {
            ...
            if (
              ...
              !token.hidden &&
              !(
                (token?.combatant && token?.combatant.defeated) ||
      ...
      if (setting("round-chatmessages") && combat && game.user.isTheGM) {
        if (combatStarted) {
          ChatMessage.create(
            { user: game.user, flavor: "Round Start" },
            { roundmarker: true }
          );
        } else if (Object.keys(delta).some((k) => k === "round")) {
          await ChatMessage.create(
            { user: game.user, flavor: `Round ${delta.round}` },
            { roundmarker: true }
          );
        }
      }
      ...
      if (
        setting("play-round-sound") &&
        setting("round-sound") &&
        Object.keys(delta).some((k) => k === "round")
      ) {
      ...
      if (
        combat &&
        combat.started &&
        (delta.round || delta.turn) &&
        setting("auto-scroll")
      ) {
        $(
          `#sidebar #combat-tracker li[data-combatant-id="${combat.current.combatantId}"]`
        ).each(function () {
          $(this).parent().scrollTop(Math.max(this.offsetTop, 0)); // - $(this).height()
        });
        $(
          `#combat-popout #combat-tracker li[data-combatant-id="${combat.current.combatantId}"]`
        ).each(function () {
          $(this)
            .parent()
            .scrollTop(Math.max(this.offsetTop - 96, 0)); // - $(this).height()
        });
      }
    });
    ...
    Hooks.on("preUpdateToken", (document, update, options, userId) => {
      if (
        ...
        CombatTurn.showShadow(
          document.object,
          document.object.x,
          document.object.y
        );
        MonksCombatDetails.emit("showShadows", {
          uuid: document.uuid,
          x: document.object.x,
          y: document.object.y,
        });
    ...
    Hooks.on("sightRefresh", function () {
      for (let [id, shadow] of Object.entries(CombatTurn.shadows)) {
        ...
        if (token) {
          ...
          shadow.visible = canvas.effects.visibility.testVisibility(
            { x: shadow.x, y: shadow.y },
            { tolerance, object: token }
          );
        }
      }
    });
  ...
  static async showShadow(token, x, y) {
    ...
    let tokenImage = await loadTexture(
      token.document.texture.src || "icons/svg/mystery-man.svg"
    );
  ...
  static ready() {
    game.settings.settings.get("monks-combat-details.play-turn-sound").default =
      !game.user.isGM; //(game.user.isGM ? 0 : 60); //set the default when we have the users loaded
    game.settings.settings.get("monks-combat-details.play-next-sound").default =
      !game.user.isGM;
    game.settings.settings.get("monks-combat-details.clear-targets").default =
      game.user.isGM;
    ...
    if (setting("showcurrentup") && !game.user.isGM) {
      if (setting("large-print")) {
        $("#your-turn")
          .addClass("current")
          .removeClass("next")
          .html(i18n("MonksCombatDetails.Turn"))
          .addClass("show");
        window.setTimeout(() => {
          $("#your-turn").removeClass("show current");
        }, 2000);
      } else ui.notifications.warn(i18n("MonksCombatDetails.Turn"));
    }

    // play a sound
    if (setting("play-turn-sound") && setting("turn-sound") != "") {
      //volume() > 0 && !setting("disablesounds") &&
      //let volume = (setting('volume') / 100) * game.settings.get("core", 'globalInterfaceVolume');
    ...
    if (setting("shownextup") && !game.user.isGM) {
      if (setting("large-print")) {
        $("#your-turn")
          .addClass("next")
          .removeClass("current")
          .html(i18n("MonksCombatDetails.Next"))
          .addClass("show");
        window.setTimeout(() => {
          $("#your-turn").removeClass("show next");
        }, 2000);
      } else ui.notifications.info(i18n("MonksCombatDetails.Next"));
    }
    // play a sound
  ...
  static checkCombatTurn(combat) {
    ...
    if (combat && combat.started) {
      ...
      if (next == undefined || next >= combat.turns.length) next = null;
      ...
      if (entry !== undefined) {
        if (isActive) {
          ...
          if (
            game.modules.get("hidden-initiative")?.active &&
            combat.round == 1 &&
            !game.user.isGM
          )
            //If hidden initiatives is active, then don't show up next information
            return;
