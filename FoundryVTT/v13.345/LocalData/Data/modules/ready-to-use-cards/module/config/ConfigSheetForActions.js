 ...
 */
export class ConfigSheetForActions extends BaseApplicationV2 {
	...
	static DEFAULT_OPTIONS = {
		...
		form: {
			closeOnSubmit: true,
			handler: ConfigSheetForActions.#onSubmit
		}
	}
	/** @override */
	static PARTS = {
	...
	constructor(object={}, options={}) {
		...
		this.#stacks = computeAllPossibleStackList().map( s => {
	...
	#prepareStackForGUI() {
		...
		const stacks = this.#stacks.map( stack => {
			const data = {
	...
	async _prepareContext(_options) {
		return {
			stacks: this.#prepareStackForGUI()
		};
	}

	/** @override */
	_onRender(_context, _options) {
		this.bindEvent('.declared-deck .toggle-button.deck.active', this.#onClickToggleDeck);
		this.bindEvent('.declared-deck .toggle-button.show.active', this.#onClickToggleDetails);

		this.bindEvent('.details.action-group .group-action.toggle-button.active', this.#onClickToggleActionChoice);
		this.bindEvent('.details.action-group .group-check.toggle-button.active', this.#onClickToggleWholeActionGroup);
		this.bindEvent('.details.action-group .group-fold.toggle-button.active', this.#onClickFoldActionGroup);
		this.bindEvent('.details.action-group .group-tab', this.#onClickChangeActionGroupTab);

		this.bindEvent('.details.action-group .param-input.button-text', this.#onChangeActionButtonText, {event: "change"} );
		this.bindEvent('.details.action-group .param-input.real-param', this.#onChangeActionParameter, {event: "change"} );
		this.bindEvent('.details.action-group .param-boolean-input', this.#onChangeActionBooleanParameter);

		this.bindEvent('.details.core-params .toggle-button.active', this.#onClickToggleCoreParameter);
		this.bindEvent('.details.core-params .param-input', this.#onClickEditCoreParameter, {event: "change"} );
	}

	/** @override */
	static async #onSubmit(event, form, formData) {
		const wholeDetails = this.#stacks.filter( s => {
	...
	async #onClickToggleActionChoice(event) {
		...
		const stack = this.#stacks.find( s => s.key === deckKey);
	...
	async #onClickToggleWholeActionGroup(event) {
	...
	async #onClickFoldActionGroup(event) {
	...
	async #onClickChangeActionGroupTab(event) {
	...
	async #onChangeActionButtonText(event) {
	...
	async #onChangeActionParameter(event) {
	...
	async #onChangeActionBooleanParameter(event) {
	...
	async #onClickToggleDeck(event) {
		event.preventDefault();
		const a = event.currentTarget;
		const key = a.parentElement.parentElement.dataset.key;

		const stack = this.#stacks.find( s =>s.key === key );
	...
	async #onClickToggleDetails(event) {
		event.preventDefault();
		const a = event.currentTarget;
		const key = a.parentElement.parentElement.dataset.key;
	...
	async #onClickToggleCoreParameter(event) {
		event.preventDefault();
	...
	async #onClickEditCoreParameter(event) {
		event.preventDefault();
		const input = event.currentTarget;
