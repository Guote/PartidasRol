...
export class CustomCardStack {
    ...
    get sortedCardList() {
        ...
        const cards = (this.isDeckStack ? this.stack.availableCards : Array.from(this.stack.cards.values()) )
            .filter( c => this.isHandledByRTUC(c) );
        cards.sort( (a,b) => {
    ...
    isHandledByRTUC(card) {
        return !! (new CustomCardStack(card.source)).coreStackRef;
    }
     ...
     */
    get handledByModule() {
        return !!this.stack.getFlag("ready-to-use-cards", "owner");
    }
    ...
    get manuallyRegistered() {
        return !!this.stack.getFlag("ready-to-use-cards", "registered-as");
    }
     ...
     */
     async sendMessageForStacks(flavor, stacks) {
        ...
        const html = await foundry.applications.handlebars.renderTemplate(template, preparedData);
