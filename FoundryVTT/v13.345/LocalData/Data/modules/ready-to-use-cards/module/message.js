...
/**
 * Alter displayed message depending on rights
 * @param message
...
export const alterCardMessage = (message, html) => {
    ...
    if( hideContent ) {
        const htmlMsg = html.querySelector('.rtucards-message');
        if(htmlMsg) { htmlMsg.outerHTML= '';}
    } else {
        html.querySelectorAll('.rtucards-message .card-link').forEach(cardLink => {
            cardLink.addEventListener("click", event => onClickShowCard(event));
        });
    }
}
