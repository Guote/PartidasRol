...
Hooks.once("ready", async () => {
  ...
  registerGlobalTypes();
  game.animabfguote13 ??= {};
  game.animabfguote13.api ??= {};
  Object.assign(game.animabfguote13.api, { ABFAttackData });
  game.socket.on("system.animabfguote13", async (p) => {
...
Hooks.on("renderChatMessage", async (message, html) => {
  ...
  const flags = message.flags?.animabfguote13 ?? {};
  const targets = Array.isArray(flags.targets) ? [...flags.targets] : [];
  const rowId = `#animabfguote13-defense-row-${message.id}`;
...
Hooks.on("renderTokenHUD", async (hud, html) => {
  ...
  if (!actor) return;
  const flagSystem = game.animabfguote13.id;
