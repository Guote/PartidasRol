...
function _getBestEffectKey(effects, rolledValue) {
  ...
  const keys = Object.keys(effects).map((k) => Number(k)).filter((n) => Number.isFinite(n)).sort((a, b) => a - b);
...
async function _rollPsychicPotential(actor, power) {
  ...
  const mentalPatternImbalance = false;
...
async function _sendPowerToChat({
  ...
  const speaker = token ? ChatMessage.getSpeaker({ actor, token }) : ChatMessage.getSpeaker({ actor });
...
async function _sendPsychicAttackToChat({
  ...
  await ABFAttackData.builder().attackAbility(roll.total).damage(Number(baseDamage) || 0).ignoreArmor(false).reducedArmor(0).armorType(power.system?.critic?.value ?? game.animabfguote13.weapon.NoneWeaponCritic.NONE).damageType(game.animabfguote13.combat.DamageType.NONE).presence(0).isProjectile(true).damagesEnergy(effectData?.affectsInmaterial?.value).automaticCrit(false).critBonus(0).attackerId(actor.id).weaponId(power.id).targets([]).build().toChatMessage({ actor, weapon: power });
}
async function castPsychicPower(sheet, event) {
  ...
  if (combatType === "defense") {
    ...
    const defaultAbilityFormula = "@psychic.psychicProjection.imbalance.defensive.base.value";
    const shieldData = ABFSupernaturalShieldData.builder().name(`${power.name} (${difficultyKey ?? rolledValue})`).shieldPoints(shieldPoints).abilityFormula(defaultAbilityFormula).flags({ animabfguote13: { supernaturalShield: { type: "psychic" } } }).build();
...
async function castPsychicPowerDifficulty(sheet, event) {
  const powerId = event.currentTarget.dataset.powerId;
  ...
  if (combatType === "defense") {
    const shieldPoints = Number(shieldValueCheck(effectText) ?? 0) || 0;
    const defaultAbilityFormula = "@psychic.psychicProjection.imbalance.defensive.base.value";
    const shieldData = ABFSupernaturalShieldData.builder().name(`${power.name} (${difficultyKey})`).shieldPoints(shieldPoints).abilityFormula(defaultAbilityFormula).flags({ animabfguote13: { supernaturalShield: { type: "psychic" } } }).build();
    await actor.newSupernaturalShield(shieldData);
    await _sendPowerToChat({
      actor,
      token,
