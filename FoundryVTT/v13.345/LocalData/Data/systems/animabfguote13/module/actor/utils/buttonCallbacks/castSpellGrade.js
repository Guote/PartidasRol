...
async function castSpellGrade(sheet, event) {
  ...
  if (combatType === "defense") {
    ...
    await actor.newSupernaturalShield(
      ABFSupernaturalShieldData.builder().name(`${spell.name} (${localizeGrade(grade)})`).shieldPoints(shieldPoints).abilityFormula(abilityFormula).flags({ animabfguote13: { supernaturalShield: { type: "mystic" } } }).build()
    );
    return;
  }
  if (useDialog) {
    const token = sheet.token?.document ?? sheet.token ?? actor.getActiveTokens()[0]?.document;
  ...
  await ABFAttackData.builder().attackAbility(roll.total).damage(baseDamage).ignoreArmor(false).reducedArmor(0).armorType(spell.system.critic?.value ?? game.animabfguote13.weapon.NoneWeaponCritic.NONE).damageType(game.animabfguote13.combat.DamageType.NONE).presence(0).isProjectile(true).automaticCrit(false).critBonus(0).attackerId(actor.id).weaponId(spell.id).targets([]).build().toChatMessage({ actor, weapon: spell });
}
