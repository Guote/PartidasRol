...
import { calculateDamage } from "../combat/utils/calculateDamage.js";
...
import "../items/ABFItem.js";
class ABFActor extends Actor {
  i18n = game.i18n;
  /**
  ...
  async rollAbility(ability, sendToChat = true) {
    ...
    let formula = `${this.system.general.diceSettings.abilityDie.value} + ${abilityValue} + ${mod ?? 0}`;
    if (abilityValue >= 200)
  ...
  async newSupernaturalShield(shieldData) {
    const itemCreateData = typeof shieldData?.toItemCreateData === "function" ? shieldData.toItemCreateData() : shieldData;
  ...
  async deleteSupernaturalShield(supShieldId) {
    ...
    if (supShield) {
      ...
      executeMacro(supShield.name ?? void 0, args);
    }
  }
  /**
   ...
   */
  async evaluatePsychicFatigue(power, psychicDifficulty, eliminateFatigue, sendToChat = true) {
    const {
      psychic: {
  ...
  async psychicShieldsMaintenance(revert) {
    ...
    for (const psychicShield of psychicShields) {
      const psychic = psychicShield.getFlag(game.animabfguote13.id, "psychic");
      if (psychic?.overmantained) {
        if (psychic.maintainMax >= psychicShield.system.shieldPoints) {
          psychicShield.unsetFlag(game.animabfguote13.id, "psychic");
        } else {
          const supShield = {
          ...
          this.applyDamageSupernaturalShield(supShield.id, damage, false, void 0);
        }
      }
    }
  }
  ...
  accumulateDefenses(keepAccumulating) {
    const defensesCounter = this.getFlag(game.animabfguote13.id, "defensesCounter") || {
      accumulated: 0
    };
    const newDefensesCounter = defensesCounter.accumulated + 1;
    if (keepAccumulating) {
      this.setFlag(game.animabfguote13.id, "defensesCounter", {
        accumulated: newDefensesCounter,
        keepAccumulating
      });
    } else {
      this.setFlag(game.animabfguote13.id, "defensesCounter.keepAccumulating", keepAccumulating);
    }
  }
  /**
  ...
  resetDefensesCounter() {
    const defensesCounter = this.getFlag(game.animabfguote13.id, "defensesCounter");
    if (defensesCounter === void 0) {
      this.setFlag(game.animabfguote13.id, "defensesCounter", {
        accumulated: 0,
        keepAccumulating: true
      });
    } else {
      this.setFlag(game.animabfguote13.id, "defensesCounter.accumulated", 0);
    }
  }
  /**
   ...
   */
  mysticCanCastEvaluate(spell, spellGrade, casted = { prepared: false, innate: false }, override = false) {
    ...
    spellCasting.canCast.prepared = this.system.mystic.preparedSpells.find(
    ...
    const innateMagicValue = innateMagic.via.length !== 0 && innateVia ? innateVia.system.final.value : innateMagic.main.final.value;
  ...
  async consumeMaintainedZeon(revert) {
    ...
    const updatedZeon = revert ? zeon.value + zeonMaintained.value : zeon.value - zeonMaintained.value;
    return this.update({
      system: {
        mystic: {
  ...
  deletePreparedSpell(spellName, spellGrade) {
    let preparedSpellId = this.system.mystic.preparedSpells.find(
      (ps) => ps.name == spellName && ps.system.grade.value == spellGrade && ps.system.prepared.value == true
    )._id;
    if (preparedSpellId !== void 0) {
  ...
  async updateInnerItem({ type, id, name, system = {} }, forceSave = false) {
    ...
    if (item) {
      const hasChanges = forceSave || !!name && name !== item.name || JSON.stringify(system) !== JSON.stringify(item.system);
      if (hasChanges) {
        if (name) {
          item.name = name;
        }
