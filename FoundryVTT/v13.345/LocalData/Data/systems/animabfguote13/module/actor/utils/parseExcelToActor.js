...
import { calculateRegenerationTypeFromConstitution } from "./prepareActor/calculations/actor/general/calculations/calculateRegenerationTypeFromConstitution.js";
import { calculateAttributeModifier } from "./prepareActor/calculations/util/calculateAttributeModifier.js";
import "../../types/domine/TechniqueItemConfig.js";
import { INITIAL_MENTAL_PATTERN_DATA } from "../../types/psychic/MentalPatternItemConfig.js";
const parseExcelToActor = async (excelData, actor) => {
  ...
  if (excelVersionSplitted.length > 0) {
    ...
    if (!requiredExcelVersions.some((item) => item === excelVersion)) {
      const versionWarning = game.i18n.localize("anima.ui.importDataFromExcelWarning") + requiredExcelVersions.join(", ");
  ...
  SetEmptyIfUndefined(excelData.Armas_Conocidas).split(",").map((value) => value.trim()).filter((element) => element !== "");
  ...
  ).split(",").map((value) => value.trim()).filter((element) => element !== "");
  ...
  SetEmptyIfUndefined(excelData.TécnicasKi).split(",").map((value) => value.trim()).filter((element) => element !== "");
  ...
  const viasMagia = separarNivelDeVia(
    SetEmptyIfUndefined(excelData.VíasDeMagiaSeleccionadas).split(",").map((value) => value.trim()).filter((element) => element !== "")
  );
...
function separarHabilidadesKi(habilidades) {
  ...
  const habilidadesSinSellos = indexSellos !== -1 ? habilidades.slice(0, indexSellos).trim() : habilidades.trim();
...
function expandAtaqueElemental(text) {
  if (!text) return "";
  return text.replace(/Ataque elemental\s*\(([^)]+)\)/gi, (_match, group) => {
    const elements = group.split(",").map((e) => e.trim()).filter((e) => e !== "");
