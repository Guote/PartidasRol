  ...
  return {
    ...
    attacker: {
      ...
      mystic: {
        ...
        critic: game.animabfguote13.weapon.NoneWeaponCritic.NONE,
        resistanceEffect: { value: 0, type: void 0, check: false },
        visible: false,
        distanceCheck: false,
        projectile: {
...
class CombatAttackDialog extends FormApplication {
  constructor(attacker, defender, hooks, options = {}) {
    ...
    if (psychicPowers.length > 0) {
      ...
      psychic.critic = power?.system.critic.value ?? game.animabfguote13.weapon.NoneWeaponCritic.NONE;
    }
    if (spells.length > 0) {
      ...
      mystic.critic = spell?.system.critic.value ?? game.animabfguote13.weapon.NoneWeaponCritic.NONE;
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["animabfguote13-dialog combat-attack-dialog no-close"],
      submitOnChange: true,
      closeOnSubmit: false,
      width: null,
      height: null,
  ...
  activateListeners(html) {
    ...
    html.find(".send-attack").click(async () => {
      ...
      this.attackerActor.setFlag(game.animabfguote13.id, "lastOffensiveWeaponUsed", weaponUsed);
      if (typeof damage !== "undefined") {
        ...
        if (weapon !== void 0 && criticSelected !== game.animabfguote13.weapon.NoneWeaponCritic.NONE && criticSelected == weapon?.system.critic.secondary.value) {
        ...
        const critic = criticSelected ?? game.animabfguote13.weapon.WeaponCritic.IMPACT;
    ...
    html.find(".send-mystic-attack").click(async () => {
      ...
      if (spellUsed) {
        ...
        this.attackerActor.setFlag(game.animabfguote13.id, "lastOffensiveSpellUsed", spellUsed);
    ...
    html.find(".send-psychic-attack").click(async () => {
      ...
      if (powerUsed) {
        ...
        this.attackerActor.setFlag(game.animabfguote13.id, "lastOffensivePowerUsed", powerUsed);
        let combatModifier = 0;
        for (const key in attackerCombatMod) {
          combatModifier += attackerCombatMod[key]?.value ?? 0;
        }
  ...
  getData() {
    ...
    if (combat.unarmed) {
      ...
      ui.weaponHasSecondaryCritic = weapon.system.critic.secondary.value !== game.animabfguote13.weapon.NoneWeaponCritic.NONE;
  ...
  async _updateObject(event, formData) {
    ...
    if (Object.prototype.hasOwnProperty.call(formData, "attacker.mystic.spellUsed") && formData["attacker.mystic.spellUsed"] === "") {
      delete formData["attacker.mystic.spellUsed"];
    }
    if (Object.prototype.hasOwnProperty.call(formData, "attacker.mystic.spellGrade") && formData["attacker.mystic.spellGrade"] === "") {
    ...
    if (prevSpell !== this.modalData.attacker.mystic.spellUsed) {
      ...
      this.modalData.attacker.mystic.critic = spell?.system.critic.value ?? game.animabfguote13.weapon.NoneWeaponCritic.NONE;
    ...
    if (prevPower !== this.modalData.attacker.psychic.powerUsed) {
      ...
      this.modalData.attacker.psychic.critic = power?.system.critic.value ?? game.animabfguote13.weapon.NoneWeaponCritic.NONE;
    }
    this.render();
  }
}
