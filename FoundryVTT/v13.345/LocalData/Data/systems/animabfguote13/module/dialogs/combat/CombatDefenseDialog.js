...
const getInitialData = (attacker, defender) => {
  ...
  const defensesCounter = defenderActor.getFlag(game.animabfguote13.id, "defensesCounter") || {
    accumulated: 0,
...
class CombatDefenseDialog extends FormApplication {
  constructor(attacker, defender, hooks) {
    ...
    if (supernaturalShields.length > 0) {
      const mysticShield = supernaturalShields.find(
        (w) => w.flags?.animabfguote13?.supernaturalShield?.type === "mystic"
      );
      if (mysticShield) {
      ...
      const psychicShield = supernaturalShields.find(
        (w) => w.flags?.animabfguote13?.supernaturalShield?.type === "psychic"
      );
      if (psychicShield) {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["animabfguote13-dialog combat-defense-dialog no-close"],
      submitOnChange: true,
      closeOnSubmit: false,
      width: 525,
      height: 240,
  ...
  activateListeners(html) {
    ...
    html.find(".send-defense").click(async (e) => {
      ...
      this.defenderActor.setFlag(game.animabfguote13.id, "lastDefensiveWeaponUsed", weaponUsed);
    ...
    html.find(".send-mystic-defense").click(async () => {
      ...
      if (!newShield) {
        ...
        this.defenderActor.setFlag(game.animabfguote13.id, "lastDefensiveSpellUsed", spellUsed);
    ...
    html.find(".send-psychic-defense").click(async () => {
      ...
      if (!newShield) {
        ...
        this.defenderActor.setFlag(game.animabfguote13.id, "lastDefensivePowerUsed", powerUsed);
