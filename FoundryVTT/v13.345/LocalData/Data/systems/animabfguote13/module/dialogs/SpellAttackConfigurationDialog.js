...
class SpellAttackConfigurationDialog extends FormApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["animabfguote13-dialog", "spell-attack-config-dialog"],
      submitOnChange: true,
      closeOnSubmit: false,
      resizable: true,
  ...
  async _sendAttack() {
    ...
    try {
      ...
      const mpFinal = Number(
        actor.system?.mystic?.magicProjection?.imbalance?.offensive?.final?.value ?? mpBase
      );
      const die = mpBase >= 200 ? actor.system?.general?.diceSettings?.abilityMasteryDie?.value : actor.system?.general?.diceSettings?.abilityDie?.value;
      const roll = new ABFFoundryRoll(`${die} + ${mpFinal} + ${mod}`, actor.system);
      await roll.evaluate({ async: true });
      const token = attacker.token?.object ?? attacker.token ?? null;
      const speaker = token ? ChatMessage.getSpeaker({ token }) : ChatMessage.getSpeaker({ actor });
      const gradeLabel = game.i18n.localize(`anima.ui.mystic.spell.grade.${grade}.title`);
      await roll.toMessage({
        speaker,
        flavor: `${spell.name} (${gradeLabel})`
      });
      const attackData = ABFAttackData.builder().attackAbility(roll.total).damage(finalDamage).ignoreArmor(false).reducedArmor(0).armorType(
        spell.system?.critic?.value ?? game.animabfguote13.weapon.NoneWeaponCritic.NONE
      ).damageType(game.animabfguote13.combat.DamageType.NONE).presence(0).isProjectile(true).automaticCrit(false).critBonus(0).attackerId(actor.id).weaponId(spell.id).targets([]).build();
...
export {
  SpellAttackConfigurationDialog
};
