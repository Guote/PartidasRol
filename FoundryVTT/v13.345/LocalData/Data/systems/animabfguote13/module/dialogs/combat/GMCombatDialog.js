...
import { executeMacro } from "../../utils/functions/executeMacro.js";
...
class GMCombatDialog extends FormApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["animabfguote13-dialog gm-combat-dialog"],
      submitOnChange: true,
      closeOnSubmit: false,
      height: 600,
      width: 700,
      template: "systems/animabfguote13/templates/dialog/combat/gm-combat-dialog.hbs",
      title: "GM Combat"
    });
  }
  get attackerActor() {
  ...
  get isDamagingCombat() {
    ...
    const isPhysicalDamagingCombat = attacker.result?.type === "combat" && attacker.result.values.damage !== 0;
    const isMysticDamagingCombat = attacker.result?.type === "mystic" && attacker.result.values.damage !== 0;
    const isPsychicDamagingCombat = attacker.result?.type === "psychic" && attacker.result.values.damage !== 0;
  ...
  get canApplyDamage() {
    ...
    const hasDamage = calculations.damage !== void 0 && calculations?.damage > 0;
  ...
  activateListeners(html) {
    ...
    html.find(".roll-resistance").click(async () => {
      ...
      const resistance = this.defenderActor.system.characteristics.secondaries.resistances[type].base.value;
  ...
  getData() {
    ...
    if (attacker.result && defender.result) {
      ...
      attacker.result.values.total = attacker.result.values.initialTotal - attacker.result.values.modifier + attackerModifier;
      ...
      defender.result.values.total = defender.result.values.initialTotal - defender.result.values.modifier + defenderModifier;
      ...
      if (this.isDamagingCombat) {
        ...
        const finalBaseDamage = (attacker.result.values.damage ?? 0) - (this.defenderActor.system.combat.damageReduction.final.value ?? 0);
  ...
  mysticCastEvaluateIfAble() {
    if (this.modalData.attacker.result?.type === "mystic") {
      const { spellCasting, spellName, spellGrade } = this.modalData.attacker.result.values;
      this.attackerActor.mysticCast(spellCasting, spellName, spellGrade);
    }
    if (this.modalData.defender.result?.type === "mystic") {
      const { spellCasting, spellName, spellGrade, supShield } = this.modalData.defender.result.values;
      if (supShield.create) {
  ...
  async newSupernaturalShieldIfBeAble() {
    ...
    if ((result?.type === "mystic" || result?.type === "psychic") && supShield.create) {
      ...
      const abilityFormula = result.type === "psychic" ? "@psychic.psychicProjection.imbalance.defensive.final.value" : "@mystic.magicProjection.imbalance.defensive.final.value";
      const shieldData = ABFSupernaturalShieldData.builder().name(result.type === "psychic" ? result?.power?.name : result?.spell?.name).shieldPoints(shieldPoints).abilityFormula(abilityFormula).flags({
        animabfguote13: {
          supernaturalShield: { type: result.type }
          // 'mystic' | 'psychic'
        }
      }).build();
  ...
  applyDamageSupernaturalShieldIfBeAble(supShieldId) {
    ...
    if (defenderIsWinner && (this.modalData.defender.result?.type === "mystic" || this.modalData.defender.result?.type === "psychic") && !immuneToDamage) {
  ...
  executeCombatMacro(resistanceRoll) {
    ...
    if (attacker.result?.type === "combat") {
      const weaponName = attacker.result.values?.weaponName || attacker.result.weapon?.name || "Unarmed";
