...
class ABFAttackData {
  ...
  constructor(p = {}) {
    ...
    this.armorType = p.armorType ?? game.animabfguote13.weapon.NoneWeaponCritic.NONE;
    this.damageType = p.damageType ?? game.animabfguote13.combat.DamageType.NONE;
    ...
    this.visibilityConditions = Array.isArray(p.visibilityConditions) ? [...p.visibilityConditions] : [];
    ...
    this.targets = Array.isArray(p.targets) ? p.targets.map(ABFAttackData._normalizeTarget) : [];
  }
  // Comments in English
  static _normalizeTarget(t = {}) {
    return {
      ...
      // 'pending'|'rolling'|'done'|'expired'
      rolledBy: t.rolledBy ?? "",
      defenseResult: t.defenseResult ?? null,
      label: t.label ?? t.actorName ?? void 0,
  ...
  static _resolveTokenForActor(actor) {
    try {
      ...
      const selected = canvas.tokens.placeables?.find(
        (t) => t.isHovered || t.isVisible && t.document?.id && canvas.tokens.controlled.length === 0 && t.document?.actorId === actor.id
      );
  ...
  async toChatMessage(opts = {}) {
    const actor = opts.actor ?? (this.attackerId ? game.actors?.get?.(this.attackerId) : null);
    if (!actor) {
      ui.notifications?.warn("Actor no encontrado para el mensaje de ataque.");
      return;
    }
    const weapon = opts.weapon ?? (this.weaponId ? actor.items?.get?.(this.weaponId) : null);
    if (!weapon) {
...
class ABFAttackDataBuilder {
  ...
  armorType(t) {
    this._p.armorType = t ?? game.animabfguote13.weapon.NoneWeaponCritic.NONE;
    return this;
  }
  // Damage typing
  damageType(t) {
    this._p.damageType = t ?? game.animabfguote13.combat.DamageType.NONE;
    return this;
  }
