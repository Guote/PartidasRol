...
function isProjectileAttack(attackData) {
  const projectileType = attackData?.projectile?.type ?? attackData?.projectileType ?? null;
  if (attackData?.isProjectile === true) return true;
  return projectileType === "shot" || projectileType === "throw" || projectileType === "projectile";
}
...
function getDefensesCounter(actor) {
  return actor?.getFlag?.(game.animabfguote13.id, "defensesCounter") ?? {
    accumulated: 0,
    keepAccumulating: true
  };
}
async function autoRollDefenseAgainstAttack({
  defenderToken = null,
  defenderActor = null,
  attackData,
  defenseMod = 0
  ...
  const multipleDefensePenalty = candidate.applyMultipleDefensePenalty ? multipleDefensePenaltyFromAccumulated(accumulated) : 0;
  const projectilePenalty = isProjectileAttack(attackData) ? candidate.projectilePenalty : 0;
  const die = candidate.naturalBase >= 200 ? actor.system?.general?.diceSettings?.abilityMasteryDie?.value ?? "1d100xa" : actor.system?.general?.diceSettings?.abilityDie?.value ?? "1d100xa";
  ...
  const flavorKey = candidate.type === "block" ? "chat.defense.autoRollFlavor.block" : candidate.type === "dodge" ? "chat.defense.autoRollFlavor.dodge" : "chat.defense.autoRollFlavor.supernaturalShield";
  ...
  const speaker = defenderToken ? { ...ChatMessage.getSpeaker({ token: defenderToken }), alias: tokenName } : ChatMessage.getSpeaker({ actor });
  ...
  const defenseTypeNormalized = candidate.type === "supernaturalShield" ? "shield" : candidate.type;
  const defenseData = ABFDefenseData.builder().defenseAbility(roll.total).armor(taFinal).inmodifiableArmor(false).defenseType(defenseTypeNormalized).defenderId(actor.id).defenderTokenId(defenderToken?.id ?? "").weaponId(candidate.weaponId ?? "").shieldId(candidate.shieldId ?? "").stackDefense(candidate.stackDefense).applyMultipleDefensePenalty(candidate.applyMultipleDefensePenalty).projectilePenalty(projectilePenalty).build();
