...
const Migration11UpdatePsychicPowersStructuredData = {
  id: "migration_update-psychic-powers-structured-data",
  version: "2.0.9",
  order: 2,
  title: "Update psychic powers to use structured data",
  description: "Updates psychic powers to the new structured data format by copying the system data from the official compendium (animabfguote13.psychic) while preserving custom description and effect texts. Ensures default structured fields exist per effect (damage, fatigue, shieldPoints, affectsInmaterial) and tries to parse them from effect text.",
  filterItems(item) {
    return item.type === "psychicPower" && !["animabfguote13.psychic"].includes(item.pack);
  },
  filterActors(actor) {
...
export {
  Migration11UpdatePsychicPowersStructuredData
};
