...
const Migration10UpdateSpellsStructuredData = {
  id: "migration_update-spells-structured-data",
  version: "2.0.9",
  order: 1,
  title: "Update spells to use structured data",
  description: "Updates spells to the new structured data format by copying the system data from the official compendium (animabfguote13.magic) while preserving custom descriptions. Ensures default structured fields exist (damage, area, resistanceEffect, shieldPoints).",
  filterItems(item) {
    return item.type === "spell" && !["animabfguote13.magic"].includes(item.pack);
  },
  filterActors(actor) {
