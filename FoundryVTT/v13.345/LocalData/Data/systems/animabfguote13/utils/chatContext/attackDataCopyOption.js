function attackDataCopyOption() {
  return {
    ...
    condition: (li) => {
      ...
      return msg?.getFlag(game.animabfguote13.id, "kind") === "attackData" && !!msg.getFlag(game.animabfguote13.id, "attackData");
    },
    // Copy JSON from flags (with fallback)
    callback: async (target) => {
      const id = target.closest("li.chat-message")?.dataset.messageId;
      const data = id && game.messages.get(id)?.getFlag(game.animabfguote13.id, "attackData");
...
export {
  attackDataCopyOption as default
};
