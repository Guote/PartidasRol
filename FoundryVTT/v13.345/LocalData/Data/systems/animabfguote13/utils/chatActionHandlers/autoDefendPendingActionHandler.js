...
async function autoDefendPendingActionHandler(message, _html, ds) {
  try {
    ...
    const targets = msg.getFlag(game.animabfguote13.id, "targets") ?? [];
    ...
    for (const t of pendings) {
      ...
      if (!actor) continue;
      if (!game.user.isGM && !actor.testUserPermission?.(game.user, CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER)) {
        continue;
      }
...
const action = "auto-defend-pending";
...
function entryFromAuto(r, tok) {
  const tokenUuid = r.token?.document?.uuid ?? tok?.document?.uuid ?? r.token?.uuid ?? tok?.uuid ?? tok?.id ?? "";
  ...
  return {
    ...
    label: label ?? r.token?.name ?? tok?.name ?? r.actor.name,
    // final fallback
...
export {
  action,
  autoDefendPendingActionHandler as default
};
