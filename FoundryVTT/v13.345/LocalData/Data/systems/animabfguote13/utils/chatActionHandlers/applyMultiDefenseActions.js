async function applyMultiEntry(message, _html, ds) {
  try {
    ...
    if (!(amount > 0)) {
      const base = Number.isFinite(Number(ds.base)) ? Number(ds.base) : Number(entry.damageFinal ?? 0);
    ...
    if (entry.applied) {
      const ok2 = await Dialog.confirm({
        ...
        content: `<p>${game.i18n.localize?.("chat.result.confirmBody") ?? "Este objetivo ya recibió daño de este resultado. ¿Seguro?"}</p>
                  <p><b>${amount}</b> ${game.i18n.localize?.("chat.result.points") ?? "puntos"}</p>`,
        yes: () => true,
        no: () => false
      });
      if (!ok2) return;
    ...
    await msg.setFlag(game.animabfguote13.id, "entries", entries);
...
applyMultiEntry.action = "animabfguote13-apply-multi-entry";
async function applyMultiRemaining(message, _html, ds) {
  try {
    const msg = game.messages.get(ds.messageId ?? message?.id);
    if (!msg) return ui.notifications?.warn("Mensaje no encontrado.");
    const entries = msg.getFlag(game.animabfguote13.id, "entries") ?? [];
    for (let i = 0; i < entries.length; i++) {
      ...
      if (!(dmg > 0)) continue;
      const actor = e.tokenId ? canvas.tokens.get(e.tokenId)?.actor : game.actors.get(e.actorId);
      if (!actor) continue;
...
applyMultiRemaining.action = "animabfguote13-apply-multi-remaining";
const handlers = {
  "animabfguote13-apply-multi-entry": applyMultiEntry,
  "animabfguote13-apply-multi-remaining": applyMultiRemaining
};
