...
class CombatAttackDialog extends FormApplication {
  constructor(attacker, defender, hooks, options = {}) {
    ...
    if (psychicPowers.length > 0) {
      ...
      psychic.critic = power?.system.critic.value ?? game.animabf.weapon.NoneWeaponCritic.NONE;
    }
    if (spells.length > 0) {
      ...
      mystic.critic = spell?.system.critic.value ?? game.animabf.weapon.NoneWeaponCritic.NONE;
  ...
  activateListeners(html) {
    ...
    html.find(".send-attack").click(async () => {
      ...
      if (typeof damage !== "undefined") {
        ...
        if (weapon !== void 0 && criticSelected !== game.animabf.weapon.NoneWeaponCritic.NONE && criticSelected == weapon?.system.critic.secondary.value) {
  ...
  getData() {
    ...
    if (combat.unarmed) {
      ...
      ui.weaponHasSecondaryCritic = weapon.system.critic.secondary.value !== game.animabf.weapon.NoneWeaponCritic.NONE;
  ...
  async _updateObject(event, formData) {
    ...
    if (prevSpell !== this.modalData.attacker.mystic.spellUsed) {
      ...
      this.modalData.attacker.mystic.critic = spell?.system.critic.value ?? game.animabf.weapon.NoneWeaponCritic.NONE;
    ...
    if (prevPower !== this.modalData.attacker.psychic.powerUsed) {
      ...
      this.modalData.attacker.psychic.critic = power?.system.critic.value ?? game.animabf.weapon.NoneWeaponCritic.NONE;
    }
    this.render();
  }
}
