...
class SpellAttackConfigurationDialog extends FormApplication {
  ...
  async _sendAttack() {
    ...
    try {
      ...
      ).damageType(game.animabf.combat.DamageType.NONE).presence(0).isProjectile(true).automaticCrit(false).critBonus(0).attackerId(actor.id).weaponId(spell.id).targets([]).build();
