...
async function castSpellGrade(sheet, event) {
  ...
  if (combatType === "defense") {
    ...
    await actor.newSupernaturalShield(
      ABFSupernaturalShieldData.builder().name(`${spell.name} (${localizeGrade(grade)})`).shieldPoints(shieldPoints).abilityFormula(abilityFormula).flags({ animabf: { supernaturalShield: { type: "mystic" } } }).build()
    );
    return;
  }
  if (useDialog) {
