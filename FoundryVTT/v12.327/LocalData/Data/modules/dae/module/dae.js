...
export var enhanceStatusEffects;
...
function flagChangeKeys(actor, change) {
    ...
    if (hasSkillBonus) {
        const skillPassive = change.key.match(/flags.skill-customization-5e.(\w\w\w).skill-bonus/);
        const abl = skillPassive[1];
        console.error(`dae | deprecated change key ${change.key} found in ${actor.name} use system.skills.${abl}.bonuses.check instead`);
        return;
    }
...
async function myAddCEEffectWith(effectData, uuid, origin, overlay, isToken) {
    ...
    //@ts-expect-error
    if (foundry.utils.isNewerVersion("6.9", game.modules?.get("dfreds-convenientEffects")?.version)) {
...
export async function evalArgs({ effectData, item, context, actor, change, spellLevel = 0, damageTotal = 0, doRolls = false, critical = false, fumble = false, whisper = false, itemCardUuid = null }) {
...
export async function doEffects(item, activate, targets = undefined, options = {
    whisper: false, spellLevel: 0, damageTotal: null, itemCardUuid: null, critical: false,
    ...
    if (options.itemCardId) {
        foundry.utils.logCompatibilityWarning("itemCardId is deprecated, pass itemCardUuid instead", { since: "12.0.22", until: "13.0.1" });
        if (!options.itemCardUuid) {
            const itemCard = game.messages?.get(options.itemCardId);
            options.itemCardUuid = itemCard?.uuid;
        }
    }
...
export async function applyNonTransferEffects(item, activate, targets, options = { whisper: false, spellLevel: 0, damageTotal: null, itemCardUuid: null, critical: false, fumble: false, tokenId: undefined, effectsToApply: [], removeMatchLabel: false, toggleEffect: false, selfEffects: "none" }) {
    if (!targets)
        return;
    let macroLocation = "mixed";
    let appliedEffects = [];
    ...
    if (localTargets.length > 0) {
        const result = await applyActiveEffects({ activate, targetList: localTargets, activeEffects: appliedEffects, effectDuration: item.system.duration, itemCardUuid: options.itemCardUuid, removeMatchLabel: options.removeMatchLabel, toggleEffect: options.toggleEffect, metaData: options.metaData, origin: options.origin });
    }
}
...
export function daeInitActions() {
    ...
    Hooks.on("createActiveEffect", (effect, data, options, userId) => {
        ...
        if (effect.parent instanceof Actor) {
            ...
            processConditionalEffects(effect.parent, {}, options, userId);
        }
        else if (effect.transfer) {
            const item = effect.parent;
            const actor = item?.parent;
            if (actor)
                processConditionalEffects(actor, {}, options, userId);
        }
    });
    ...
    Hooks.on("updateActiveEffect", (effect, updates, options, userId) => {
        if (game.user !== game.users?.activeGM)
            return;
        if (effect.parent instanceof Actor) {
            const actor = effect.parent;
            processConditionalEffects(effect.parent, updates, options, userId);
        }
        else if (effect.transfer) {
            const item = effect.parent;
            const actor = item?.parent;
            if (actor)
                processConditionalEffects(actor, updates, options, userId);
        }
    });
...
async function applyRequiredStatusEffectsHook(effect, options, userId) {
    ...
    try {
        ...
        await busyWait(0.05); // Hopefull this will allow the rest of the updates to complete
