...
class ABFActor extends Actor {
  ...
  async newSupernaturalShield(type, power, psychicDifficulty, spell, spellGrade) {
    ...
    if (type === "psychic") {
      ...
      const baseEffect = shieldBaseValueCheck(potentialBaseDifficulty, power?.system.effects) ?? 0;
    ...
    if (supernaturalShieldData.psychic.overmantained) {
      item.setFlag("animabf-guote", "psychic", supernaturalShieldData.psychic);
    }
  ...
  async psychicShieldsMaintenance(revert) {
    ...
    for (const psychicShield of psychicShields) {
      const psychic = psychicShield.getFlag("animabf-guote", "psychic");
      if (psychic?.overmantained) {
        if (psychic.maintainMax >= psychicShield.system.shieldPoints) {
          psychicShield.unsetFlag("animabf-guote", "psychic");
        } else {
          const supShield = {
  ...
  accumulateDefenses(keepAccumulating) {
    const defensesCounter = this.getFlag("animabf-guote", "defensesCounter") || {
      accumulated: 0,
      keepAccumulating
    };
    const newDefensesCounter = defensesCounter.accumulated + 1;
    if (keepAccumulating) {
      this.setFlag("animabf-guote", "defensesCounter", {
        accumulated: newDefensesCounter,
        keepAccumulating
      });
    } else {
      this.setFlag("animabf-guote", "defensesCounter.keepAccumulating", keepAccumulating);
    }
  }
  /**
  ...
  resetDefensesCounter() {
    ...
    if (defensesCounter === void 0) {
      ...
      this.setFlag("animabf-guote", "defensesCounter.accumulated", 0);
    }
  }
  /**
