...
const getInitialData = (attacker, defender) => {
  ...
  const defensesCounter = defenderActor.getFlag("animabf-guote", "defensesCounter") || {
    accumulated: 0,
...
class CombatDefenseDialog extends FormApplication {
  ...
  activateListeners(html) {
    ...
    html.find(".send-defense").click(async (e) => {
      ...
      this.defenderActor.setFlag("animabf-guote", "lastDefensiveWeaponUsed", weaponUsed);
    ...
    html.find(".send-mystic-defense").click(async () => {
      ...
      if (!newShield) {
        ...
        this.defenderActor.setFlag("animabf-guote", "lastDefensiveSpellUsed", spellUsed);
    ...
    html.find(".send-psychic-defense").click(async () => {
      ...
      let formula = `1d100xa + ${psychicProjection}[Proy. Psíquica] + ${combatModifier}[Mod.]`;
      if (this.modalData.defender.withoutRoll) {
        formula = formula.replace("1d100xa", "0");
      }
      ...
      if (!newShield) {
        ...
        this.defenderActor.setFlag("animabf-guote", "lastDefensivePowerUsed", powerUsed);
