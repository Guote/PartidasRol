  ...
  const initialData = {
    ...
    attacker: {
      ...
      mystic: {
        ...
        spellUsed: macroCookies?.mystic?.spellUsed ?? void 0,
        ...
        damage: {
          special: macroCookies?.mystic?.damage ?? 0,
          final: 0
        },
      ...
      psychic: {
        ...
        powerUsed: macroCookies?.psychic?.powerUsed ?? void 0,
        ...
        damageModifier: macroCookies?.psychic?.damage ?? 0,
...
class CombatAttackDialog extends FormApplication {
  ...
  activateListeners(html) {
    ...
    html.find(".send-attack").click(async () => {
      ...
      this.attackerActor.setFlag("animabf-guote", "lastOffensiveWeaponUsed", weaponUsed);
    ...
    html.find(".send-mystic-attack").click(async () => {
      ...
      if (spellUsed) {
        ...
        this.attackerActor.setFlag("animabf-guote", "lastOffensiveSpellUsed", spellUsed);
    ...
    html.find(".send-psychic-attack").click(async () => {
      ...
      if (powerUsed) {
        ...
        this.attackerActor.setFlag("animabf-guote", "lastOffensivePowerUsed", powerUsed);
        let combatModifier = 0;
        for (const key in attackerCombatMod) {
          combatModifier += attackerCombatMod[key]?.value ?? 0;
        }
        let rollModifiers = [psychicProjection, /* modifier,  */combatModifier];
        let formula = getFormula({
          values: rollModifiers,
          labels: ["Proy. Psi.", "Mod."],
        });
