...
export const calculateWeaponDamage = (weapon, data) => {
  if (weapon.data.isSummon) {
    return weapon.data.damage.base.value;
  }
  const getDamage = () => {
    const weaponStrengthModifier = calculateWeaponStrengthModifier(
      weapon,
      data
    );
    const extraDamage = data.general.modifiers.extraDamage.value;
    if (
      weapon.data.isRanged.value &&
      weapon.data.shotType.value === WeaponShotType.SHOT
    ) {
      const { ammo } = weapon.data;
      if (ammo) {
        let ammoDamage =
          ammo.data.damage.final.value - ammo.data.quality.value * 2;
    ...
    return (
      addSizeModifier(weapon, weapon.data.damage.base.value) +
      weaponStrengthModifier +
      extraDamage +
      weapon.data.quality.value * 2
    );
  };
